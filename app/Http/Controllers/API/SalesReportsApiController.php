...
class SalesReportsApiController extends Controller
{
    public function getSalesReport(Request $request)
    {
        ...
        if (!empty($request->startDate) && !empty($request->endDate)) {
            $SalesReports->whereBetween('order_items.created_at', [$startDate, $endDate]);
        }

        if (!empty($request->seller)) {
            $SalesReports->where('order_items.seller_id', $request->seller);
        }

        if (!empty($request->category)) {
            $SalesReports->where('products.category_id', $request->category);
        }

        if (!empty($request->deliveryBoy)) {
            $SalesReports->where('orders.delivery_boy_id', $request->deliveryBoy);
        }

        if (!empty($request->payment_type)) {
            if ($request->payment_type == "1") {
                $SalesReports->where('orders.payment_method', "COD");
            } else {
                $SalesReports->where('orders.payment_method', '!=', "COD");
            }
        }
    ...
    public function excelSalesReport(Request $request)
    {
        ...
        $csvData = [];
        $csvData[] = ['Product Name', 'Sub Total']; // Corrected header

        foreach ($SalesReports as $row) {
            $csvData[] = [$row->product_name, $row->sub_total]; // Fixed amount
        }

        // Generate CSV file
        $csvFileName = 'sales_export_' . Str::random(10) . '.csv';
